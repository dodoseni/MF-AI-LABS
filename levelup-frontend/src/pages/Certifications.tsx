...
export default function Certifications() {
  ...
  // "Add certification" and the "Start tracking" / "Mark completed" status
  // transitions below are frontend-only interactions — there is no backend
  // write endpoint for certifications (see docs/CHANGELOG.md), so we keep a
  // local, editable copy and mutate it directly rather than writing back to
  // the API.
  const [certifications, setCertifications] = useState<Certification[]>([])
  // Seed `certifications` from whichever API response we haven't seeded
  // from yet (covers the initial load and any later retry) without
  // clobbering in-progress local edits on unrelated re-renders. This
  // "adjust state during render" pattern (see the React docs on derived
  // state) avoids a redundant effect + extra render pass just to copy a
  // freshly-fetched value into local state.
  const [seededFrom, setSeededFrom] = useState<Certification[] | null>(null)
  if (certificationsRes.data && certificationsRes.data !== seededFrom) {
    setSeededFrom(certificationsRes.data)
  ...
  return (
    <div>
      ...
      <ApiState loading={certificationsRes.loading} error={certificationsRes.error} onRetry={certificationsRes.retry}>
