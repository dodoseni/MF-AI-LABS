...
## 2026-09-03 — MIKK-54: Final QA of LevelUp frontend-to-backend integration (validates MIKK-52, branch agent/frontend-developer/cfd450f93fb7, commit 7871eda)

- Component: QA
- Issue/ref: MIKK-54 (independent verification of MIKK-52)
- What was done:
  - `cd levelup-frontend && npm ci && npm run build && npm run lint`: build clean (tsc -b + vite build, 0 errors); lint exit 0 (only the same pre-existing `set-state-in-effect` / `only-export-components` warning categories already present before MIKK-52, no new warning types, no errors).
  - Confirmed via `grep` that `src/api/` only implements/calls `getProfile`/`getCertifications`/`getCareerLevels`/`getLearningPlan` — no `/api/competencies` call anywhere in `src/` (the only "competenc*" hit is an unrelated word in a certification description string in `data/mock.ts`).
  - Confirmed architecture: `context/CertificationsContext.tsx` still exists and remains the single shared source of truth for certifications + derived career-path progress; `pages/LearningPlan.tsx` and `types/index.ts` use `StudyChecklist[]`/`StudyChecklistItem` exclusively — grepped for `goals`/`weeklyPlan`/`calendar` fields in the learning-plan code path and found none (only an unrelated UI label string and a doc comment referencing the old model by name).
  - Read `client.ts`, `CertificationsContext.tsx`, `ProfileContext.tsx`, `LearningPlan.tsx` and `components/ui.tsx` (`ApiNotice`): every API-backed resource seeds from local mock/fallback data first, fetches on mount, and on failure keeps the fallback + shows an `ApiNotice` error banner with a working Retry button — confirmed the "never blank" behavior is structurally guaranteed (state is never unset, only conditionally replaced).
  - **Production `VITE_API_BASE_URL` — MERGE BLOCKER.** Inspected `.github/workflows/levelup-dev.yml`: no `env:` block anywhere, `VITE_API_BASE_URL` is never referenced. Inspected `src/api/client.ts`: no hardcoded fallback URL — `import.meta.env.VITE_API_BASE_URL ?? ''`, i.e. an *empty* base URL, not the production backend URL. Empirically reproduced the exact GH Actions build (`unset VITE_API_BASE_URL && npm run build`) and confirmed the compiled bundle bakes in the "VITE_API_BASE_URL is not configured" error path — Vite inlines env vars at build time, so this cannot be fixed post-build/at runtime. Net effect: the deployed Azure Static Web App will **always** run in fallback/mock mode with a persistent error banner on every page; it will never reach the real backend, because neither the workflow injects the variable nor does the client code fall back to a real URL. Per the issue's own classification rule (neither workflow injection nor a safe production-URL fallback exists) → **PRODUCTION VITE_API_BASE_URL: NOT CONFIGURED**, classified **MERGE BLOCKER**.
    - Required fix (not applied — out of this issue's scope, and the workflow must not be modified by QA): add a GitHub Actions **repository (or environment) variable** named `VITE_API_BASE_URL` = `https://levelup-api-dev-dfgugzd7a0fvf4f7.swedencentral-01.azurewebsites.net`, then add to `.github/workflows/levelup-dev.yml`'s "Build and deploy" step:
      ```yaml
            - name: Build and deploy
              ...
              with:
                ...
      ```
  - No accidental hardcoding: `grep -rn azurewebsites levelup-frontend/src` only matches a doc comment in `vite-env.d.ts` (not executable code); the real hostname lives only in `.env.example` and `README.md`.
  - `REQUIREMENT_CERT_LINKS` (in `CertificationsContext.tsx`) cross-checked label-by-label against `backend/src/data/careerLevels.js` (all 23 non-holistic requirement labels across Level 1–3) and `backend/src/data/certifications.js` (all 23 mapped `certId`s exist) — 1:1 exact match, no silently-incorrect mapping, unmapped requirements safely fall back to the backend's static `met`/`detail`. Missing backend `certId` field on `/api/career-levels` requirements classified **NON-BLOCKING TECH DEBT** (frontend workaround is correct and documented; recommend Data Engineer add `certId` to the contract as a follow-up).
  - Verified remaining local/mock-only functionality matches MIKK-52's own report: certification add/delete/status writes (local `useState`, no backend write endpoints), Learning Plan checklist/item add/toggle/delete (same), `Profile.tsx`'s "Active learning plans" card (still reads `developmentGoals` from mock, unconnected to `StudyChecklist[]`), and the AI Assistant (`pages/AiAssistant.tsx`, fully simulated, no backend endpoint).
  - CORS: live-curled the deployed backend (`OPTIONS /api/profile` with `Origin: http://localhost:5173`) → `204`, `Access-Control-Allow-Origin: *`; also confirmed `GET /api/profile` → `200` and `GET /api/competencies` → `404` directly against the live Azure backend.
  - Scope check: `git diff <merge-base>..HEAD --stat` touches only `levelup-frontend/**` and `docs/CHANGELOG.md` — no `backend/`, `.github/workflows/`, SQL, Azure infra, AI/Foundry, or auth changes.
  - Mergeability: branch is **behind** current `origin/develop` by one merged PR (MIKK-51, Prisma foundation — `backend/` only), but a scratch `git merge --no-commit --no-ff` of this branch into fresh `origin/develop` completes with **zero conflicts** (only `docs/CHANGELOG.md` auto-merges cleanly, both sides appended in different spots). `git merge-tree --write-tree` also succeeds cleanly.
- Result: **FAIL** — one MERGE BLOCKER (production `VITE_API_BASE_URL` never configured, confirmed by reproducing the actual CI build). Everything else (build, lint, API integration, architecture, loading/error/fallback UX, `REQUIREMENT_CERT_LINKS`, CORS, scope, merge-cleanliness) passes.
- Recommendation: **DO NOT MERGE** until the GitHub variable + workflow `env:` change above is made (or an equivalent fix). This is a small, mechanical CI/infra fix, not a code defect — the frontend code itself is correct and ready. Flagging for Full-Stack Developer to add the workflow `env:` line (touches `.github/workflows/`, outside this branch's/QA's scope) and Product Owner to add the repo variable.
- Open questions / risks:
  - Once the workflow fix lands, re-run this same production-fallback check (`unset VITE_API_BASE_URL && npm run build`, inspect bundle) against the *fixed* workflow's actual build context to confirm the variable is really injected end-to-end (a GH Actions repo variable can still fail to reach the Oryz/SWA build container depending on how `Azure/static-web-apps-deploy@v1` invokes `app_build_command`; worth a real CI run, not just a local repro).
  - Carried over from MIKK-52: `certId` missing from `/api/career-levels` contract (non-blocking), `Profile.tsx` Active Learning Plans still on mock `developmentGoals`.
