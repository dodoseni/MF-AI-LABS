...
## 2026-09-03 — MIKK-51: Phase 1 — Prisma Foundation (install & configure Prisma for Azure SQL, no models yet)

- Component: backend
- Issue/ref: MIKK-51 (Phase 1 only, per the reviewed planning doc attached to the issue; Phase 2 — `DatabaseSmokeTest`/`GET /api/db-test` — is a separate, gated follow-up issue, not started here)
- What was done:
  - Confirmed via inspection that Prisma did not already exist on `develop` (`f40678a`/`d17dd94`): pre-change `backend/package.json` deps were `dependencies: { cors: ^2.8.5, express: ^4.19.2 }`, `devDependencies: { jest: ^29.7.0, supertest: ^7.0.0 }` — no Prisma, `mssql`, `tedious`, or any DB client.
  - Added `prisma@6.19.3` (dev) and `@prisma/client@6.19.3` (runtime) to `backend/package.json`/`package-lock.json`. Pinned to the latest **6.x** rather than the newly-released `7.x`/`8.0.0-rc` lines: `7.x` pulls in an optional sub-dependency (`@prisma/streams-local`) requiring Node ≥22, which would conflict with this repo's `engines.node: ">=20"` (and produce `npm warn EBADENGINE`); `6.19.3` supports Node ≥18.18 with a clean `npm ci`.
  - Ran `npx prisma init --datasource-provider sqlserver`, then hand-simplified the result to match this plain-CommonJS-JS repo: kept `backend/prisma/schema.prisma` (`datasource db { provider = "sqlserver", url = env("DATABASE_URL") }`, `generator client { provider = "prisma-client-js" }` — the classic generator with Prisma's default output path, not the new `prisma-client`/custom-`output` generator Prisma 6's `init` scaffolds by default, which needs a different output-path `.gitignore` entry and is more ESM-oriented). Removed the auto-generated `prisma.config.ts` (a TypeScript file `prisma init` now scaffolds by default) and its accompanying `.env` — neither is needed for `prisma validate`/`generate` to find `prisma/schema.prisma` via its conventional default path, and a `.ts` config file didn't fit a repo with no TypeScript tooling. No models added (Phase 2 boundary).
  - Added `backend/.env.example` (documents the `DATABASE_URL` name only, no value) and `backend/prisma/README.md` (Prisma CLI command reference: `validate`/`generate`/`migrate dev`/`migrate deploy`, and local schema validation without a live DB).
  - Added a new "Database (Prisma + Azure SQL) — foundation only" section to `backend/README.md` covering: current status (installed/configured, not wired into any repository yet — `Certification`/`CareerLevel`/`Profile`/`learning-plan` are untouched and still mock-data-backed), the required `DATABASE_URL` shape (`sqlserver://host:1433;database=db;user=...;password=...;encrypt=true`, `encrypt=true` mandatory for Azure SQL TLS), the migration strategy (`migrate dev` vs `migrate deploy`), and the two open questions below stated explicitly as unresolved. Updated the project-structure listing and the "Azure App Service deployment notes" section to reflect `DATABASE_URL` being unused today.
  - Verified: `npm ci` succeeds cleanly in `backend/` (392 packages, no `EBADENGINE` warnings); `npx prisma validate` passes with a placeholder `DATABASE_URL` set locally and correctly fails (schema-validation error, no connection attempt) with `DATABASE_URL` unset; `npm test` — 6 suites / 12 tests, unchanged and still green (no existing test needed to change; this issue adds no runtime behavior).
- Decision/notes:
  - Auth mode is **SQL Authentication** (username/password in the connection string), per explicit instruction — not Managed Identity. No `@prisma/adapter-mssql` or other driver-adapter package added; Prisma's native `sqlserver` connector is sufficient for both queries and future migrations under this auth mode.
  - Did not create any `Project`/`Projects` entity/table (MIKK-38's unrelated, unmerged prior attempt used a different stack — raw `mssql` + Managed Identity — and is not reused or referenced here), did not touch `/api/learning-plan` or its repository (MIKK-44 territory), did not migrate `Certification`/`CareerLevel`/`Profile` off mock data, and did not add the `DatabaseSmokeTest` model or `GET /api/db-test` endpoint (Phase 2, tracked separately) or touch `.github/workflows/levelup-api-dev.yml`.
  - New dev-only vulnerabilities from `npm audit`: `prisma`/`@prisma/config`/`deepmerge-ts` (3 high, CLI-tooling-only, no fix currently available upstream for 6.19.3; not part of the runtime `@prisma/client` code path). Pre-existing `express`/`body-parser`/`qs` findings (3 moderate) were confirmed present on `develop` before this change (verified via `git stash`) and are unrelated to this issue.
- Open questions / risks (carried forward from the planning doc's §5/§6, not re-derived):
  1. **App Service vs. self-hosted VM (unresolved, explicitly not assumed).** The only backend deployment path that exists in this repo is `.github/workflows/levelup-api-dev.yml` → Azure App Service (`levelup-api-dev`) via `azure/webapps-deploy@v3`. An earlier planning conversation described the backend as running on a self-hosted VM instead — no VM deployment configuration exists anywhere in this repo. This inspection cannot confirm which is correct, or whether both exist in parallel. This blocks deciding how `DATABASE_URL` reaches the runtime and where `prisma migrate deploy` would run.
  2. **Exact format of the `SqlConnectionString` Key Vault secret (unresolved — cannot be inspected from this runtime).** Azure Portal's Key Vault connection string for SQL Database is typically ADO.NET-style (`Server=tcp:...;Initial Catalog=...;User ID=...;Password=...`), not Prisma's JDBC-style URL. No live credentials were requested, hardcoded, or committed anywhere in this issue — the actual stored format needs inspecting (not guessing) before `DATABASE_URL` can be populated for real.
  3. **How the runtime authenticates to Key Vault to retrieve the secret (unresolved, depends on #1).** Candidates: an App Service Key Vault reference in Application Settings (if App Service is the real target), or VM Managed Identity + `@azure/identity`/`@azure/keyvault-secrets` at startup / a deploy-time `az keyvault secret show` script / manual-CI-injected env var (if a VM is the real target). No mechanism is implemented or assumed.
  4. **Where `prisma migrate deploy` actually runs (unresolved, depends on #1).** No step was added to `levelup-api-dev.yml`, and no VM deploy script was invented — there's no existing CI/CD path to a VM in this repo to hook into.
  5. **Local development story** — whether developers need a real/containerized SQL Server locally, or whether local dev stays out of scope until connectivity is proven end-to-end (Phase 2), is still open.
  6. **Prisma major-version churn risk** — `prisma`/`@prisma/client` 7.x/8.x (release candidate) are already out; 6.19.3 was chosen deliberately for Node 20 compatibility, but this pin will need revisiting if the repo's Node baseline is raised or when a stable 7.x/8.x lands.
...
## 2026-09-03 — MIKK-52: Fresh LevelUp frontend-to-backend integration against current develop

- Component: frontend
- Issue/ref: MIKK-52 (fresh integration; explicitly does not reuse the stale MIKK-35/PR#13 branch, which predates MIKK-36/MIKK-37 and the MIKK-46 Learning Plan contract redesign)
- What was done:
  - New `levelup-frontend/src/api/` client layer: `client.ts` (generic `apiGet<T>()` + `ApiError`, unwraps the backend's `{ "data": T }` envelope, reads `VITE_API_BASE_URL`), plus `profile.ts`, `certifications.ts`, `careerLevels.ts`, `learningPlan.ts` — one thin wrapper per endpoint (`GET /api/profile`, `/api/certifications`, `/api/career-levels`, `/api/learning-plan`). `GET /api/competencies` is not implemented or called anywhere (confirmed via a live Playwright network trace — zero requests to it).
  - `VITE_API_BASE_URL` env var: added `levelup-frontend/.env.example` (pointing at the deployed dev backend), `src/vite-env.d.ts` for typed `import.meta.env`, and `.gitignore` entries for `.env`/`.env.local`/`.env.*.local`. No Azure hostname is hardcoded in application code.
  - New `context/ProfileContext.tsx`: seeds `Profile` from `GET /api/profile`, falls back to `data/mock.ts`'s `currentUser` (now typed as `Profile`, with `id`/`email` added to match the backend contract) while loading or on error, exposes `status` (`loading`/`success`/`error`) + `refetch()`. `Sidebar`, `Dashboard` (greeting) and `Profile` page now consume this instead of importing `currentUser` directly.
  - `context/CertificationsContext.tsx` **preserved as the single shared source of truth** for certifications + derived career-path progress (not deleted or bypassed, per the issue's explicit instruction) — only its *initial* state source changed: `certifications` now seeds from `GET /api/certifications` and career-level structure now seeds from `GET /api/career-levels`, both falling back to `data/mock.ts` while loading/on error. `addCertification`/`deleteCertification`/`updateCertificationStatus` are unchanged and stay local-only (no backend write endpoints exist). Added an `editedRef` guard so a slow/late API response can never overwrite a certification the user already added/edited locally.
  - `pages/LearningPlan.tsx`: initial `checklists` state now seeds from `GET /api/learning-plan` (`StudyChecklist[]`, per the MIKK-46 contract) instead of `data/mock.ts` directly, with the same fallback + "don't clobber local edits" guard as above. Toggle/add/delete item and create/delete plan interactions are unchanged and remain frontend-local.
  - Loading/error/fallback handling for every API-backed resource, applied consistently: state always starts as the local mock/fallback value (page never blank) → fetch on mount → on success replace with live data → on failure keep the fallback and show a new `ApiNotice` banner (`components/ui.tsx`, with a `spinner`/banner CSS added to `app.css`) with a **Retry** button. Wired into Dashboard, Profile, Certifications, Career Path and Learning Plan. New i18n keys (`common.retry`, `common.loading*`, `common.error*`) added for both `en`/`no`.
  - `types/index.ts`: added a `Profile` interface matching the `GET /api/profile` shape; widened `Certification.progress`/`earnedDate` to `| null` since the backend always sends explicit `null` rather than omitting these fields.
  - `README.md`: new "API integration" section documenting the env var, the 4 API-backed resources + their consuming pages, the loading/error/fallback pattern, how `CertificationsContext` ownership was preserved, and a "Remaining local-only functionality & mock data" list; updated the component-architecture tree and removed stale references to the already-deleted Competency page.
- Decision/notes:
  - **Backend contract gap**: `GET /api/career-levels` requirements don't carry a `certId` linking a requirement to a `Certification.id` (the frontend's own `data/mock.ts` template always had this field). Bridged with a static `REQUIREMENT_CERT_LINKS` label→certId map in `CertificationsContext.tsx` (documented inline) so live progress/met-state can still be derived from the certifications list; requirements with no map entry fall back to the backend's own static `met`/`detail` rather than being incorrectly forced to "not met". **Flagging for Data Engineer/Full-Stack Developer**: consider adding `certId` to the `/api/career-levels` contract directly so the frontend doesn't need this label-matching workaround.
  - Chose "seed once, then local-only" over polling/refetch-on-every-mutation: since there are no write endpoints, re-fetching after a local edit would either be a no-op or (worse) silently discard the user's local change — the `editedRef` guard makes this explicit and intentional rather than accidental.
  - Did not touch `Profile.tsx`'s "Active learning plans" card (still reads `developmentGoals` from mock — a pre-existing gap flagged in the MIKK-37 entry above, out of this issue's explicit scope) or the AI Assistant (fully simulated, no backend endpoint).
- CORS: tested the frontend locally (`npm run dev`, `VITE_API_BASE_URL` pointed at the live Azure dev backend) — succeeded with no CORS errors. The backend's `app.js` calls Express `cors()` with no options (`Access-Control-Allow-Origin: *`), confirmed via a manual `curl -i -X OPTIONS`/`GET` with an `Origin: http://localhost:5173` header. No backend change needed or made.
- Validation:
  - `npm ci && npm run build` (tsc -b + vite build) → clean, 0 errors.
  - `npm run lint` (oxlint) → exit 0; only pre-existing-pattern warnings (`react/set-state-in-effect` on data-fetching `useEffect`s, `react/only-export-components` on context files exporting both a provider and a hook) — no errors, no new categories vs. the codebase's existing warnings.
  - No frontend test runner is configured in `package.json` (no `test` script) — none to run.
  - Live verification via a local Playwright/Chromium session against `npm run dev` pointed at the deployed dev backend (`https://levelup-api-dev-dfgugzd7a0fvf4f7.swedencentral-01.azurewebsites.net`): Dashboard, Profile, Certifications, Career Path (Level 1-4) and Learning Plan (StudyChecklist) all render live data with zero console/page errors; network trace confirmed calls only to `/api/profile`, `/api/certifications`, `/api/career-levels`, `/api/learning-plan` and **zero** calls to `/api/competencies`; toggling a Learning Plan checklist item worked end-to-end (2/7 → 1/7, local-only).
  - Repeated the same session with `VITE_API_BASE_URL` pointed at a non-existent host to simulate a backend outage: every page still rendered its full local/mock data, each showed the correct `ApiNotice` error banner with a working Retry button, and there were zero console/page errors — confirms the "never blank on outage" requirement.
- Open questions / risks:
  - See the `certId`/`REQUIREMENT_CERT_LINKS` gap above — recommend adding `certId` to `/api/career-levels` requirements in a follow-up backend issue so the frontend mapping can be removed.
  - `Profile.tsx`'s "Active learning plans" card is still disconnected from the new StudyChecklist model (same open question carried over from the MIKK-37 entry) — unchanged by this issue.
## 2026-09-03 — MIKK-53: Phase 2 — Azure SQL smoke test (`DatabaseSmokeTest` model + `GET /api/db-test`)

- Component: backend
- Issue/ref: MIKK-53 (Phase 2, gated on MIKK-51 Phase 1 — Prisma Foundation, which is implemented/QA-approved but not yet merged to `develop`). Continued on Phase 1's existing branch `agent/back-end-developer/363dd7daf6ba` @ `8228c6e` (checked out locally as `agent/back-end-developer/2eee0154001b`, same commit history — no second branch, no rebase from `develop`).
- What was done:
  - Added `model DatabaseSmokeTest { id Int @id @default(autoincrement()); message String; createdAt DateTime @default(now()) }` to `backend/prisma/schema.prisma` (additive — datasource/generator block from Phase 1 untouched).
  - Generated migration `backend/prisma/migrations/20260903110310_add_database_smoke_test/migration.sql` via `prisma migrate dev --name add_database_smoke_test`, run against a throwaway local SQL Server 2022 Docker container (`mcr.microsoft.com/mssql/server:2022-latest`) spun up solely for this task and destroyed afterward — **not** the real Azure SQL (`mikkelrev.database.windows.net`). This produces the exact, byte-real Prisma-generated SQL Server migration (not hand-authored), since a reachable dev SQL Server was available in this runtime via Docker.
  - Added `backend/src/repositories/dbTestRepository.js` (`connect()`, `create(message)`, `findById(id)` — single-purpose async functions, same convention as `certificationsRepository.js`/`careerLevelsRepository.js`), `backend/src/services/dbTestService.js` (orchestrates connect → create → read-back, distinguishing "can't connect" (`databaseConnected: false`) from "connected but insert/read failed" (`databaseConnected: true, recordCreated: false`); never throws), `backend/src/controllers/dbTestController.js`, and `backend/src/routes/dbTest.js` (`GET /api/db-test`).
  - Registered the route in `backend/src/app.js` with one additive `app.use('/api', dbTestRouter)` line (plus its `require`) — no other line changed.
  - Added `backend/tests/dbTest.test.js` with `@prisma/client` mocked at the module level (`jest.mock('@prisma/client', ...)`), covering: success path (200, exact response shape), connection-failure path (503, `databaseConnected: false`), and connected-but-write/read-failure path (503, `databaseConnected: true, recordCreated: false`). No test hits a real database.
- Decision/notes:
  - **Step 3 (apply migration to real Azure SQL): NOT done against the real database — honestly reporting this, not fabricating success.** No `DATABASE_URL`, Azure CLI, or Key Vault access is available in this runtime (confirmed: `DATABASE_URL` unset in env, no `az` CLI installed, no reachable Key Vault). Per the issue's explicit instruction, a reachable throwaway SQL Server was used instead (Docker was available) to generate a **real, Prisma-produced** migration file rather than hand-authoring one — this is a real Prisma migration, just not applied to Azure SQL. The migration was also applied and independently verified against that throwaway DB (created a row, read it back via a fresh Prisma client, then destroyed the container) to prove the model/migration/client all work correctly, before removing all trace of the throwaway DB.
  - Whichever agent/human next has real `SqlConnectionString`/`DATABASE_URL` access to `mikkelrev.database.windows.net`/`mikkelrev` should run `cd backend && npx prisma migrate deploy` to apply `20260903110310_add_database_smoke_test` for real, then hit `GET /api/db-test` to confirm.
  - `dbTestRepository.js` instantiates its own `PrismaClient` at module scope (no shared `prismaClient.js` module added — not in the issue's expected file list, and only one repository currently needs Prisma). Confirmed `new PrismaClient()` does not throw when `DATABASE_URL` is unset (Prisma connects lazily); the service's `connect()` call is what surfaces a connection failure as a structured response rather than crashing the app at startup.
  - No files for `Certification`/`CareerLevel`/`Profile`/`learning-plan` touched; no `Project`/`Projects` entity created. All 6 new/modified files are isolated and named `dbTest*` (except `schema.prisma`/`app.js`, which each got one small additive, clearly-commented block) — a full rollback checklist is in the MIKK-53 issue comment.
- Tests: `cd backend && npm ci && npm test` → 7 suites / 15 tests, all green (existing 6 suites/12 tests unchanged; 3 new tests in `dbTest.test.js`, all mocked). `npx prisma validate` with a placeholder `DATABASE_URL` still passes.
 - Open questions / risks: Carried forward from MIKK-51 — deployment target (App Service vs. self-hosted VM), exact `SqlConnectionString` secret format (ADO.NET vs. Prisma/JDBC), and how this runtime obtains `DATABASE_URL` at runtime are all still unresolved and block actually running `prisma migrate deploy` against the real Azure SQL database. This issue does not resolve them — it produces the migration and endpoint ready to run once one of those is answered.

## 2026-09-03 — MIKK-53: QA validation — Phase 2 Azure SQL smoke test (`DatabaseSmokeTest` + `GET /api/db-test`)

- Component: QA
- Issue/ref: MIKK-53 (validating the Back-end Developer's commit `80654ca` on `agent/back-end-developer/363dd7daf6ba`, on top of `8228c6e`)
...
## 2026-09-03 — MIKK-55: Add calendar section to Learning Plan page for certification journey tracking

- Component: frontend
- Issue/ref: MIKK-55
- What was done:
  - Added a new `Certification calendar` card to the Learning Plan page (new `src/components/Calendar.tsx`, month-grid view with prev/next navigation, Monday-first weeks, today/selected highlighting) that visualizes the user's certification journey: study modules, practice exams, the certification exam itself, and milestones/deadlines — each with a distinct colour + icon (module = blue/book, study session = teal/brain, practice exam = amber/target, certification exam = red/cert, milestone = violet/goal), plus a legend and a click-to-select day panel listing that day's planned activities (sorted exam → practice → milestone → module → study so the most important thing surfaces first).
  - Re-extended the `StudyChecklistItem` type (`src/types/index.ts`) with optional `date` and `type` fields, and added a new `CalendarEvent`/`CalendarEventType`/`WeeklyPlanDay` types. `LearningPlan.tsx` derives the calendar's events live from three sources so the page has one source of truth: (1) dated study-checklist items (ticking a module off doesn't remove it from the calendar — it stays as a completed activity), (2) a small hand-authored `certificationJourneyEvents` mock array (`src/data/mock.ts`) for the AZ-305 practice exam, exam date and milestone deadlines, and (3) a re-added `weeklyStudyPlan` recurring-cadence mock array, projected onto real calendar dates for a rolling window (Aug–Dec 2026) so the weekly study schedule and the calendar stay in sync instead of being two disconnected views.
  - Added a "Weekly study schedule" card next to the calendar (`grid-main-2` layout, collapses to one column on mobile) that displays the same `weeklyStudyPlan` recurring cadence in its own read-only day-by-day view — this is the literal "integrate with the weekly study schedule" requirement satisfied by sharing one data source between both UI pieces.
  - Added calendar + weekly-plan CSS to `app.css` (adapted from the pre-MIKK-37 calendar design, extended with icon-based legend/day-panel entries and a type badge per event) and 16 new EN/NO translation keys (`learning.calendar.*`, `learning.weeklyPlan.*`) in `translations.ts`; updated `learning.subtitle` to mention the calendar.
- Decision/notes:
  - MIKK-37 (2026-09-02) had removed the original `Calendar` component and its `calendarEvents`/`weeklyStudyPlan` mock data entirely when it replaced the Learning Plan model with per-certification study checklists. This issue reintroduces a calendar but deliberately does **not** revive the old `developmentGoals`-driven goals/milestones model that MIKK-37 also decoupled from Learning Plan (that model stays Profile-only, per MIKK-37's decision) — instead, calendar milestones/exam/practice events come from a small dedicated `certificationJourneyEvents` array, keeping Learning Plan's data model self-contained (study checklists + calendar/weekly-plan only).
  - Only the study-checklist items that have a `date` set appear on the calendar; items a user adds through the existing "add checklist item" input stay undated/checklist-only (no date picker was added to that flow) — this was a deliberate scope call to avoid feature creep on an already-shipped interaction; a follow-up could add an optional date field to the "add item" form if the team wants user-added items to also land on the calendar.
  - This branch was cut before MIKK-52 (frontend↔backend wiring) landed on `develop`; rebased onto it before finishing. `LearningPlan.tsx`'s `checklists` state now seeds from `GET /api/learning-plan` (MIKK-52) rather than mock data directly — the calendar derives its events from that same live `checklists` state (not from the mock import), so it correctly reflects whichever data actually loaded (API response, or the local fallback on error/while loading) with no extra work needed. `backend/src/data/learningPlan.js` (MIKK-46) still returns the plain `StudyChecklist[]` shape with no `date`/`type` fields — the new fields are optional, so items coming from the live API simply won't carry calendar dates yet (only the local mock fallback's `az-305` items do). Flagging for the Full-Stack Developer/Product Owner: if the calendar's per-item dates should also come from the API, `/api/learning-plan` would need `date`/`type` added to each item — not done here, out of this (frontend-only) issue's scope.
  - Verified with `tsc -b` (clean), `vite build` (clean), `oxlint` (only the 3 pre-existing warnings, no new ones), and a local Playwright pass: calendar renders and navigates months correctly, clicking a date updates the day panel (verified on a module day, an exam day showing both the certification-exam and a recurring study-session event sorted correctly, and an empty day), the Weekly study schedule card renders and matches the calendar's recurring events, English/Norwegian both render correctly, and the `grid-main-2` layout collapses to one column at a 420px mobile viewport with zero console/page errors throughout.
- Open questions / risks:
  - None blocking. If the Product Owner wants user-added checklist items to also support a target date (and thus appear on the calendar), that's a small, isolated follow-up to the existing "add item" modal/input in `LearningPlan.tsx`.
